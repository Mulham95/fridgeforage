...
- **Worker origin allow-list** — browser requests must come from an origin in
  `ALLOWED_ORIGINS` (`wrangler.toml`). Native/mobile requests (no `Origin`
  header) bypass this check and are gated by `APP_SHARED_SECRET` instead.
  Verified live: allowed origin → 200, unknown origin → 403.
- **CORS lockdown** — `Access-Control-Allow-Origin` echoes only the allow-listed
  origin (no wildcard), with `Vary: Origin`.
- **In-Worker per-IP rate limit** (cache-based, default 20 req/IP/min). This is
  **best-effort**: Workers' `caches.default` has propagation lag, so it catches
  sustained abuse but not tight bursts in <1 second. For reliable burst control
  add a paid Cloudflare WAF rate-limit rule or a Durable Object — the
  authoritative cost ceiling remains the Gemini spend cap.
...
1. **Set a hard spend/quota cap on the Gemini key** (Google AI Studio → Get API
   key → your project → set budget limits in the linked Google Cloud project).
   This is the single most important control — the only true ceiling on cost if
   the proxy URL is abused. The in-Worker rate limit is best-effort, not a
   billing guarantee.
2. **(Optional) Add a Cloudflare WAF rate-limit rule** on the Worker route for
   stricter burst control. Requires a paid plan; on the free tier the in-Worker
   limiter + Gemini's own per-key quota is what you get.
3. **Set `APP_SHARED_SECRET`** (`cd proxy && npx wrangler secret put
   APP_SHARED_SECRET`). Meaningful for the **mobile** app (the secret sits in
   the binary). For the **web** build it's only light friction, since the bundle
   is public — rely on the origin allow-list + (1) there.
